...
fun ForecastWeatherView(forecast: List<ForecastItem>) {
    ...
    Column(
        ...
        if (searchQuery.value.isEmpty() && hometown.isEmpty()) {
            ...
            LazyColumn(
                modifier = Modifier.fillMaxWidth() // Modifier to make the column take full width
            ) {
                // Iterate over each forecast item and display it using WeatherCard
                items(forecast) { forecastItem ->
                    // Display each forecast item using the WeatherCard composable
